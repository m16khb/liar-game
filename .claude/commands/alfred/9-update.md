...
[Step 7.2] 템플릿 상태 검증
  → [Grep] "liar-game" -n ".moai/project/product.md"
...
**보호 정책** (개선됨):
- `liar-game` 패턴 존재 → 템플릿 상태, 안전하게 덮어쓰기
...
[Step 10.5] 템플릿에 사용자 정보 주입 (병합)
  → liar-game → {project_name}
  → A liar-game project built with MoAI-ADK → {project_description}
...
[Step 11.2] 템플릿 상태 검증
  → [Grep] "liar-game" -n ".moai/config.json"
