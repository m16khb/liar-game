...
export function getModulesExcluding(rootModule: any, excludeModules: any[]): any[] {
  ...
  // 제외할 모듈들과 Admin으로 시작하는 하위 모듈들만 수집
  // (공유 모듈인 AuthModule, AnalysisModule 등은 제외하지 않음)
  ...
  return allModules.filter(module => {
    // 제외 목록에 있는지 확인
    if (excludeSet.has(module)) return false;

    // 모듈 이름으로 Admin 관련 모듈인지 확인 (추가 안전장치)
    const moduleName = module.name || '';
    if (moduleName.startsWith('Admin')) return false;
...
export function getSwaggerModules(config: SwaggerModuleConfig): any[] {
  const { rootModule, includeModules, excludeModules = [], adminModules = [] } = config;
  ...
  if (adminModules.length > 0) {
    // 관리자 모듈만 포함 (Admin으로 시작하는 모듈만 필터링)
    const allAdminModules = getMultipleModuleImports(adminModules);
    return allAdminModules.filter(module => {
      const moduleName = module.name || '';
      return moduleName.startsWith('Admin');
    });
  }
